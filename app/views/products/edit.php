<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
    <h2 class="text-lg font-semibold">Edit Product: <?= e($product->name) ?></h2>
    ...
    <div class="p-4 sm:p-6">
        <form action="<?= url("/products/{$product->id}/update") ?>" method="POST">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                ...
                <div class="flex flex-col gap-1.5 sm:col-span-2">
                    ...
                    <textarea id="description" name="description" 
                              class="w-full px-3 sm:px-3.5 py-2.5 text-sm font-sans text-slate-200 bg-input border border-border rounded-md outline-none transition-all duration-200 focus:border-accent focus:shadow-[0_0_0_3px_rgba(99,102,241,0.15)] placeholder:text-slate-500 resize-y min-h-[80px]" rows="4"><?= e($product->description ?? '') ?></textarea>
                </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row gap-3 pt-6 border-t border-border">
                <button type="submit" class="inline-flex items-center justify-center gap-1.5 px-5 py-2.5 text-sm font-medium rounded-md cursor-pointer transition-all duration-200 bg-accent text-white border border-accent hover:bg-accent-hover hover:-translate-y-px w-full sm:w-auto">💾 Update Product</button>
