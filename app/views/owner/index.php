<div class="bg-white border border-border rounded-[10px] shadow-[0_4px_24px_rgba(0,0,0,0.25)] flex items-center justify-between p-6 mb-8">
    <div>
        <h2 class="text-2xl font-bold text-black mb-1">Hello, <?= e($_SESSION['business_name'] ?? 'Owner') ?>! 👋</h2>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-8">
    <div class="bg-white border border-border rounded-[10px] p-4 sm:p-5 flex items-center gap-4 shadow-[0_4px_24px_rgba(0,0,0,0.25)] transition-all duration-200 hover:border-accent hover:-translate-y-0.5">
        ...
        <div class="flex flex-col min-w-0">
            <span class="text-xs text-black uppercase tracking-wide">Products</span>
            <span class="text-lg sm:text-xl font-bold text-slate-500"><?= number_format($totalProducts ?? 0, 0, ',', '.') ?></span>
        </div>
    </div>
    <div class="bg-white border border-border rounded-[10px] p-4 sm:p-5 flex items-center gap-4 shadow-[0_4px_24px_rgba(0,0,0,0.25)] transition-all duration-200 hover:border-accent hover:-translate-y-0.5">
        <div class="text-2xl sm:text-3xl w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-indigo-500/[0.12] rounded-md shrink-0">🛒</div>
        <div class="flex flex-col min-w-0">
            <span class="text-xs text-black uppercase tracking-wide">Orders</span>
            <span class="text-lg sm:text-xl font-bold text-slate-500"><?= number_format($totalOrders ?? 0, 0, ',', '.') ?></span>
        </div>
    </div>
    <div class="bg-white border border-border rounded-[10px] p-4 sm:p-5 flex items-center gap-4 shadow-[0_4px_24px_rgba(0,0,0,0.25)] transition-all duration-200 hover:border-accent hover:-translate-y-0.5">
        <div class="text-2xl sm:text-3xl w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-indigo-500/[0.12] rounded-md shrink-0">💸</div>
        <div class="flex flex-col min-w-0">
            <span class="text-xs text-black uppercase tracking-wide">Today's Sales</span>
            <div class="flex flex-col gap-0.5 mt-0.5">
                <span class="text-lg font-bold text-slate-500"><?= format_rupiah($todaySales ?? 0) ?> <span class="text-xs font-normal text-slate-400">Gross</span></span>
    ...
    <div class="bg-white border border-border rounded-[10px] p-4 sm:p-5 flex items-center gap-4 shadow-[0_4px_24px_rgba(0,0,0,0.25)] transition-all duration-200 hover:border-accent hover:-translate-y-0.5">
        <div class="text-2xl sm:text-3xl w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-indigo-500/[0.12] rounded-md shrink-0">📈</div>
        <div class="flex flex-col min-w-0">
            <span class="text-xs text-black uppercase tracking-wide">Monthly Revenue</span>
            <div class="flex flex-col gap-0.5 mt-0.5">
                <span class="text-lg font-bold text-slate-500"><?= format_rupiah($monthlyRevenue ?? 0) ?> <span class="text-xs font-normal text-slate-400">Gross</span></span>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <div class="bg-white border border-border rounded-[10px] shadow-[0_4px_24px_rgba(0,0,0,0.25)] overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-border flex justify-between items-center">
            <h2 class="text-base text-black font-semibold">Quick Actions</h2>
        </div>
